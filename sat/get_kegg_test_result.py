...
#     fold_dir = f'../kegg_data/fold_{fold}'
...
test_inter,test_label = [train[0,0:2]],[train[0,2]]
...
with torch.no_grad():
    ...
    for data in test_loop:
        data_mol1 = data[0].cuda()
        data_drug1 = data[1].cuda()
        torch.save(data_drug1, 'drug_kg.pt')
        data_mol2 = data[2].cuda()
        data_drug2 = data[3].cuda()
        torch.save(data_drug2, 'drug_kg2.pt')
        labels = data[4].cuda()
        # edge = data[0].to(device)
        # labels = data[1].to(device)
        # edge_index = get_edge_index(train_edges_true).to(device)
        y_pred, kg_rep1, kg_att1,kg_rep2,kg_att2 = model(data_mol1,data_drug1,data_mol2,data_drug2)
        # torch.save(mole_rep, 'drugkg_presentaiton_{}.pt'.format(str(test[i][0])))
        torch.save(kg_att1, 'drug1kg_133.pt')
        # torch.save(mole_rep2, 'drugkg_presentaiton_{}.pt'.format(str(test[i][1])))
        torch.save(kg_att2, 'drug2kg_188.pt')
        #y_pred = model(edge,labels,mole_input,sub_kg_input)
        total_pred_auc.append(y_pred.cpu().numpy().reshape(-1))

...
class SimpleGraphTransformer(nn.Module):
    def __init__(self, in_channels, d_model=128, num_heads=4, num_layers=4,
                 dim_feedforward=256, dropout=0.1, use_edge_attr=False,
                 global_pool='mean', use_cls_token=False, num_classes=1,use_global_pool=True,task_type='mole'):
        super().__init__()
        ...
        for i in range(num_layers):
            conv = TransformerConv(d_model, d_model // num_heads, heads=num_heads,
                                   dropout=dropout, edge_dim=(d_model if use_edge_attr else None))
            self.layers.append(conv)
        if use_cls_token and global_pool == 'cls':
        ...
        if global_pool == 'mean':
            ...
            raise ValueError("Unsupported global_pool type")
    def forward(self,data):
        ...
        for conv in self.layers:
            x = conv(x,edge_index)
            x = F.relu(x)
        if self.use_global_pool and self.task_type == 'mole':
            output = self.pool(x, data.batch)
        else:
            output = x[data.id.nonzero().flatten()]
        return output
...
class GraphTransformerEncoder(nn.TransformerEncoder):
    def forward(self, x, edge_index, complete_edge_index,
            ...
            ptr=None, return_attn=True):
        output = x

        for mod in self.layers:
            output,attr = mod(output, edge_index, complete_edge_index,
    ...
    #
    #     for mod in self.layers:
    #         output,attr,k_values, importance_scores = mod(output, edge_index, complete_edge_index,
    #             edge_attr=edge_attr, degree=degree,
    #             subgraph_node_index=subgraph_node_index,
    #             subgraph_edge_index=subgraph_edge_index,
    #             subgraph_indicator_index=subgraph_indicator_index,
    ...
    #     if self.norm is not None:
    #         output = self.norm(output)
    #     return output,attr,k_values, importance_scores
...
class GraphTransformer(nn.Module):
    def __init__(self,d_model, num_heads=8,
                 ...
                 global_pool='mean',task_type='mole', **kwargs):
        super().__init__()
        ...
        # self.use_edge_attr = use_edge_attr
        # if use_edge_attr:
        #     edge_dim = kwargs.get('edge_dim', 128)
        #     if edge_embed:
    ...
    def forward(self, data, return_attn=True):
        ...
        # if self.use_edge_attr and edge_attr is not None and self.task_type == 'mole':
        ...
        #new data:
        # output,attr,k_values, importance_scores = self.encoder(
        #     output,
        #     edge_index,
        #     complete_edge_index,
        #     edge_attr=edge_attr,
        ...
        if self.use_global_pool and self.task_type =='mole':
            if self.global_pool == 'cls':
                ...
                for index, drug_mole_graph in enumerate(data.to_data_list()):
                    sub_embedding = output[(data.batch == index).nonzero().flatten()]
            ...
            for index, drug_subgraph in enumerate(data.to_data_list()):
                sub_embedding = output[(data.batch == index).nonzero().flatten()]
                sub_representation.append(sub_embedding)
            output = output[data.id.nonzero().flatten()]
        # if self.max_seq_len is not None:
        #     pred_list = []
        #     for i in range(self.max_seq_len):
        #         pred_list.append(self.classifier[i](output))
        #     return pred_list
        return output, sub_representation, attr

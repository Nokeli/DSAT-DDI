...
test = np.load("../kegg_data/fold_{}/train.npy".format(args.fold))
...
model = DrugInteractionModel(number_nodes=num_nodes,in_size = 67,rel_num=13,kg_rel_num=167,num_class=1,d_model=80,dim_feedforward=80,
                dropout=0.1,num_heads=2,num_layers=2 ,batch_norm=False,abs_pe=False,
                abs_pe_dim=8,gnn_type='gine',use_edge_attr=True,num_edge_features=4,kg_num_edge_feature = 71,in_embed=False,
                edge_dim=80,k_hop=2,se='gnn',task_type='mole',device = device)
...
#             counter = 0
#             torch.save(model.state_dict(), 'SAT_DDI_add_kegg_degree_{}.pth'.format(args.fold))
#         else:
#             counter += 1
#             if counter >= patience:
...
# Testing phase
model.load_state_dict(torch.load('SAT_DDI_add_kegg_{}.pth'.format(args.fold)))
with torch.no_grad():
    model.eval()

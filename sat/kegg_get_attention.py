...
drug_file = '../kegg_data/new_smiles.txt'
SMILES = []
...
smile_graph, rel_nums = smile_to_graph('./kegg_data', SMILES)
kg_file = KnowledgeGraph('../kegg_data/new_kg.txt')
...
def subtreeExtractor(drug_id, edge_index, rel_index,shortest_paths, num_rel, fixed_num, khop,num_nodes):
    ...
    json_path = shortest_paths +'kegg'+ "subtree_fixed_" + str(fixed_num) + "_hop_" + str(khop) + "sp.json"
...
drug_list = [i for i in range(786)]
subgraph = subtreeExtractor(drug_list,edge_index,edge_type,'./',num_rel,32, 4,num_nodes)
...
def train_file(filename):
    ...
    return interactions
interactions = train_file('../kegg_data/new_ddi.txt')
...
train = np.load("../kegg_data/fold_{}/train.npy".format(args.fold))
valid = np.load("../kegg_data/fold_{}/valid.npy".format(args.fold))
test = np.load("../kegg_data/fold_{}/test.npy".format(args.fold))
# test_file = open('../data/test2.txt','r').readlines()
# test = []
# for i in test_file:
#     i= i.strip().split(' ')
...
for i in range(len(test)):
    test_inter,test_label = [test[i][0:2]],[test[i][2]]
    ...
    model = DrugInteractionModel(number_nodes=num_nodes,in_size = 67,rel_num=13,kg_rel_num=167,num_class=1,d_model=80,dim_feedforward=80,
                    dropout=0.1,num_heads=4,num_layers=2,batch_norm=False,abs_pe=False,
    ...
    #             best_auc = auroc
    #             counter = 0
    #             torch.save(model.state_dict(), 'SAT_DDI_drugbank_{}.pth'.format(args.fold))
    #         else:
    #             counter += 1
    #             if counter >= patience:
    ...
    # Testing phase
    model.load_state_dict(torch.load('SAT_DDI_kegg_att_4.pth'),strict=False)
    with torch.no_grad():
        model.eval()
        ...
        for data in test_loop:
            ...
            y_pred, mole_rep, mole_att,mole_rep2,mole_att2 = model(data_mol1,data_drug1,data_mol2,data_drug2)
            torch.save(mole_rep, 'kegg_drug_presentaiton_{}.pt'.format(str(test[i][0])))
            torch.save(mole_att, 'kegg_drug_att_{}.pt'.format(str(test[i][0])))
            torch.save(mole_rep2, 'kegg_drug_presentaiton_{}.pt'.format(str(test[i][1])))
            torch.save(mole_att2, 'kegg_drug_att_{}.pt'.format(str(test[i][1])))
            #y_pred = model(edge,labels,mole_input,sub_kg_input)
            total_pred_auc.append(y_pred.cpu().numpy().reshape(-1))
        ...
        with open('True_lable.txt','w') as f:

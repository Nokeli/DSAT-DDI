...
train = np.load("../data/fold_{}/train.npy".format(args.fold))
valid = np.load("../data/fold_{}/valid.npy".format(args.fold))
test = np.load("../data/fold_{}/test.npy".format(args.fold))
...
# with open('drugbank_test.txt','w') as f:
#     for i in range(len(test_inter)):
#         f.write(str(test_inter[i][0])+' '+str(test_inter[i][1])+'\n')
test_inter,test_label = [test[0,0:2]],[test[0,2]]
...
model = DrugInteractionModel(number_nodes=num_nodes,in_size = 67,rel_num=13,kg_rel_num=72,num_class=1,d_model=80,dim_feedforward=160,
                dropout=0.1,num_heads=2,num_layers=2,batch_norm=False,abs_pe=False,
...
# Testing phase
model.load_state_dict(torch.load('SAT_DDI_drugbank_withabpe.pth'),strict=False)
with torch.no_grad():
    model.eval()
    ...
    for data in test_loop:
        ...
        torch.save(kg_att1, 'drug1kg_598.pt')
        # torch.save(mole_rep2, 'drugkg_presentaiton_{}.pt'.format(str(test[i][1])))
        torch.save(kg_att2, 'drug2kg_597.pt')
        #y_pred = model(edge,labels,mole_input,sub_kg_input)
        total_pred_auc.append(y_pred.cpu().numpy().reshape(-1))
